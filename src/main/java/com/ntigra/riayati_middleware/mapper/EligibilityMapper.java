...
public class EligibilityMapper {
    ...
    public EligibilitySubmission toEligibilitySubmission(EligibilityRequestDto dto) {
        ...
        // ==================== BUILD ENCOUNTER ====================
        EligibilityEncounter encounter = EligibilityEncounter.builder()
                .facilityId(dto.getFacilityId())
                .type(dto.getEncounterType() != null ? dto.getEncounterType() : 1)
                .build();

        // ==================== BUILD DIAGNOSIS ====================
        List<EligibilityDiagnosis> diagnoses = null;
        if (dto.getDiagnoses() != null && !dto.getDiagnoses().isEmpty()) {
            diagnoses = dto.getDiagnoses().stream()
                    .map(diag -> {
                        ...
                        return EligibilityDiagnosis.builder()
        ...
        // ==================== BUILD ACTIVITIES ====================
        List<EligibilityActivity> activities = new ArrayList<>();
        if (dto.getActivities() != null) {
            for (var actDto : dto.getActivities()) {
                EligibilityActivity activity = EligibilityActivity.builder()
                        .id(actDto.getId())
